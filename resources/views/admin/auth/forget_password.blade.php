...
<div class="login-box">
  ...
  <div class="card card-outline card-primary">
    ...
    <div class="card-body">
      ...
      <form method="post" data-action="{{ route('admin.set.password') }}" id="set_new_password">
        @csrf
        ...
        <div class="row">
          <div class="col-8">
            <button type="submit" class="btn btn-primary" id="submit_button">Set New Password</button>
          </div>
        </div>
      </form>
      <p class="mb-1">
        <a href="{{ route('admin.login') }}">Login</a> | <a href="{{ route('admin.forget.password') }}">Forget Password</a>
      </p>
...
<script>
  $(document).ready(function(){
    $('body').on('submit','#set_new_password',function(e){
              ...
              success:function(response){
                ...
                if (data.status===200) {
                  $('#message_area').html('<div class="alert alert-success"> <i class="fas fa-check-circle"></i> '+data.message+'</div>').show();
                   $('#submit_button').text("Set New Password").prop('disabled',false);
                }else{
