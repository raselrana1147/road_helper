...
@section('title','Road Helper | Paid Users')
@section('breadcrumb')
  <div class="row mb-2">
    <div class="col-sm-6">
      <h1 class="m-0">Paid And Uncheck Users</h1>
    </div><!-- /.col -->
...
<script src="{{asset('assets/backend/plugins/bootstrap-switch/js/bootstrap-switch.min.js')}}"></script>
 ...
 <script type="text/javascript">
        ...
        var table = $('#item_table').DataTable({
            ...
            ajax: '{{ route('load_paid_uncheck_user') }}',
            columns: [
                { data: 'id', name: 'id' },
  ...
  <script>
    $(function () {
                 ...
                 $.ajax({
                      ...
                      success:function(response){
                         ...
                         var image_url=(data.avatar !==null) ? '{{asset('assets/profile_image')}}'+"/"+data.avatar : '{{asset('assets/profile_image/default.png')}}';
                            ...
                            '<dt class="col-sm-4">'+((data.is_paid  ==2) ? 'Transaction Number' : '')+'</dt>'+
                            '<dd class="col-sm-8">'+((data.is_paid  ==2) ?  data.paid_user.transaction_number : '')+'</dd>'+
      ...
      $('body').on('change','.chnagePaidStatus',function(e){
        ...
        $('body').on('click','.chnageCheckStatus',function(e){
            e.preventDefault();
               let user_id=$(this).attr('user_id');
            $.ajax({
               url:$(this).attr('data-action'),
